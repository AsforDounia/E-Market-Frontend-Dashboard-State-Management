...
const CreateProduct = () => {
    ...
    const { items: categories, loading: catLoading, error: catError, fetched } = useSelector(
        (state) => state.categories || { items: [], loading: false, error: null, fetched: false }
    );
    ...
    const handleImages = (e) => {
        ...
        if (files.length > 5) {
            errorMsg = 'You can only upload up to 5 images. The first 5 have been selected.';
        ...
        const previews = (errorMsg ? files.slice(0, 5) : files).map(file => URL.createObjectURL(file));
    ...
    return (
        <div className="max-w-3xl mx-auto px-4 py-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {catError && (
                    <Alert type="error">{catError}</Alert>
                )}

                <div>
                    <label className="block text-sm font-semibold text-gray-900 mb-2" htmlFor="categorySelect">
                        Categories
                    </label>
                    <select
                        id="categorySelect"
                    ...
                    >
                        <option value="">{catLoading ? 'Loading categories...' : 'Add a category'}</option>
                        {categoryList.map((c) => (
                    ...
                    {Array.isArray(form.categoryIds) && form.categoryIds.length > 0 && (
                        <div className="mt-3 flex flex-wrap gap-2">
                            {form.categoryIds.map((id) => {
                                ...
                                return (
                                    <div key={id} className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full flex items-center gap-2">
                ...
                <div>
                    <label className="block text-sm font-semibold text-gray-900 mb-2" htmlFor="images">
                        Product Images <span className="text-red-500">*</span>
                        <span className="text-sm font-normal text-gray-600 ml-2">(Max 5 images)</span>
                    </label>
                    <input
                        id="images"
                        type="file"
